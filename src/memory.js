/* ==========================================================================
   ...
   What it depicts: a memory consolidating. Edges strengthen when they are
   used and weaken when they are not, a retrieval walk crosses the topology
   along whatever is currently strongest, and the shape of the graph is
   different a minute later than it was at load. That is the subject of this
   site drawn rather than described.

   IT RENDERS NO DATA AND ASSERTS NOTHING. It takes no input from the
   document and writes nothing back into it: no textContent, no dataset, no
   custom event, no global. Delete the <script> tag that loads this file and
   every figure, chip, status row and word on the page is still there.

   The counts below are DELIBERATELY NOT the counts this site publishes.
   Graphonomous stores 6 node types and 17 edge types; this picture has 13
   nodes and 23 links, which are not any figure on the page and are not
   anything measured. gpscoord.com shipped a canvas animation whose
   `for (let i = 0; i < 12; i++)` was published beside it as "12 Active
   Pathfinders" — a decoration's loop bound sold as a live user metric, for
   months. The publication gate now reads the constants out of this file and
   refuses the build if any of them appears as a number in the page's text.
   ========================================================================== */
(function () {
  ...
  /* ---- IDENTITY-COUNTS: read by launch-gate.mjs. Not data. ---- */
  var NODES = 13;
  var LINKS = 23;
  var FPS = 24;
  ...
  /* Consolidation: some edges are strengthened, some are let go. The cursor
     walks the link list so the same edge is not always the one that gains. */
  var cursor = 0;
  ...
  /* Retrieval: a walk that follows whatever is strongest right now, which is
     why the path it takes changes after a consolidation. */
  var walkAt = -1;
  var walkLeft = 0;
  function walk() {
  ...
  /* prefers-reduced-motion: one frame, then stop. Not optional (SHELL.md §8.4). */
  var still =
